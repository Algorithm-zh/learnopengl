...
4.坐标到达观察空间之后，我们需要将其投影到裁剪坐标。裁剪坐标会被处理至-1.0到1.0的范围内，
  并判断哪些顶点将会出现在屏幕上。
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //模型矩阵，顶点坐标乘以这个模型矩阵会变换到世界坐标
        glm::mat4 model         = glm::mat4(1.0f); // make sure to initialize matrix to identity matrix first
        //观察矩阵，将世界坐标转化为用户视野前方的坐标而产生结果，在场景里往后移动,使物体可见,不然看不到图形
        ...
        //perspective就是创建了一个定义了可视空间的大平截头体，任何在这个平截头体以外的东西最后都不会出现在裁剪空间体积内，并且将会受到裁剪
        //一个透视平截头体可以被看作一个不均匀形状的箱子，在这个箱子内部的每个坐标都会被映射到裁剪空间上的一个点
        //第一个参数是视野，设置了观察空间的大小(表现出来的就是你看到的图像的大小), 第二个参数设置了宽高比，由视口的宽除以高， 第三四个参数是平截头体的近远平面的距离
        //下面这些参数就是通常设置的参数，可以沿用
